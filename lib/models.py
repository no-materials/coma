...
class base_model(object):
    ...
    def fit(self, train_data, train_labels, val_data, val_labels):
        t_process, t_wall = time.clock(), time.time()
        sess = tf.Session(graph=self.graph)
        shutil.rmtree(self._get_path('summaries', True), ignore_errors=True)
        writer = tf.summary.FileWriter(self._get_path('summaries', True), self.graph)
        shutil.rmtree(self._get_path('checkpoints', True), ignore_errors=True)
        os.makedirs(self._get_path('checkpoints', True))
        path = os.path.join(self._get_path('checkpoints', True), 'model')
        sess.run(self.op_init)
    ...
    def build_graph(self, M_0, F_0):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('inputs'):
                ...
                self.ph_labels = tf.placeholder(tf.float32, (self.batch_size, M_0, F_0), 'labels')
                self.ph_dropout = tf.placeholder(tf.float32, (), 'dropout')
                self.ph_z = tf.placeholder(tf.float32, (self.batch_size, self.nz[0]), 'z')
    ...
    def _get_path(self, folder, from_google_drive):
        if from_google_drive:
            return os.path.join(gd_path, folder, self.dir_name)
        else:
    ...
    def _get_session(self, sess=None):
        ...
        if sess is None:
            sess = tf.Session(graph=self.graph)
            filename = tf.train.latest_checkpoint(self._get_path('checkpoints', True))
...
class cgcnn(base_model):
    ...
    def __init__(self, L, weight_tensor, F, K, p, M, nv, which_loss, F_0=1, filter='chebyshev5', brelu='b1relu',
                 pool='mpool1',
    ...
    def upool1(self, x, p):
        ...
        if p > 1:
            x = tf.expand_dims(x, 3)  # N x M x F x 1
            x = tf.image.resize_images(x, size=[int(M * p),
                                                int(Fin)])  # , method=tf.image.ResizeMethod.BICUBIC)  # N x M x F
    ...
    def _inference(self, x, dropout):
        ...
        N, M, F = x.get_shape()
        x = tf.reshape(x, [int(N), int(M * F)])  # N x MF
        if self.M:
            with tf.variable_scope('fc'):
                x = self.fc(x, int(self.M[0]))  # N x M0

        with tf.variable_scope('fc2'):
            x = self.fc(x, int(M * F))  # N x MF

        x = tf.reshape(x, [int(N), int(M), int(F)])  # N x M x F
        ...
        with tf.name_scope('outputs'):
            x = self.filter(x, self.L[0], int(Fin), self.K[0])
